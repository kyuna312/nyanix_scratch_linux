...
int sys_sendrec(int function, int src_dest, MESSAGE* m, struct proc* p)
{
    ...
    MESSAGE* mla = (MESSAGE*)va2la(caller, m); // Get caller message

    mla->source = caller;

    assert(mla->source != src_dest); //Make sure not to send messages to yourself to prevent deadlock
     ...
     */
    if (function == SEND) {
        ret = msg_send(p, src_dest, m); // Send a message
    }
    else if (function == RECEIVE) {
        ret = msg_receive(p, src_dest, m); // Receive message
    }
    else {
...
int send_recv(int function, int src_dest, MESSAGE* msg)
{
    ...
    switch (function) {
    case BOTH: // Send first then receive
